#!/usr/bin/env bash
# E2E 冒烟测试(D14 邮箱登录):逐条验证统一鉴权闭环。每条打印 PASS/FAIL。
...
EMAIL="${E2E_EMAIL:-e2e_$(date +%j%H%M%S)@lingxi.test}"
...
echo "[RTC HTTP 仅内部令牌]"
ck "T5 直连 RTC 无 X-Internal-Token → 401" 401 \
...
login=$(curl -s -X POST -H 'Content-Type: application/json' \
  -d "{\"email\":\"$EMAIL\",\"password\":\"$PASSWD\"}" "$GW/api/v1/user/login")
TOKEN=$(printf '%s' "$login" | jstr token)
RT=$(printf '%s' "$login" | jstr refreshToken)
UID_=$(printf '%s' "$login" | jstr userId); [ -z "$UID_" ] || [ "$UID_" = "null" ] && UID_=$(jwt_sub "$TOKEN")
[ -n "$TOKEN" ] && ok "T8 登录拿到 access token (userId=$UID_)" || ng "T8 登录应返回 token" "non-empty" "empty: $login"
{ [ -n "$(printf '%s' "$login" | jstr userId)" ] && [ "$(printf '%s' "$login" | jstr userId)" != "null" ]; } \
  && ok "T8b LoginResponse.userId 非空(bug 已修)" || ng "T8b userId 应非空" "non-null" "null"

if [ -n "$TOKEN" ] && [ -n "$UID_" ]; then
  echo "[带令牌访问 + 网关注入 X-User-Id]"
  got=$(status -H "Authorization: Bearer $TOKEN" "$GW/api/v1/contact/$UID_/applyCount")
  { [ "$got" != "401" ] && [ "$got" != "000" ]; } && ok "T9 带 token 访问受保护接口 → 非401 (got=$got)" || ng "T9 带 token 应放行" "!=401/000" "$got"
  ...
  { [ "$got" != "401" ] && [ "$got" != "403" ] && [ "$got" != "000" ]; } && ok "T11 伪造 X-User-Id 被剥离 (got=$got)" \
     || ng "T11 伪造头应被网关覆盖" "!=401/403/000" "$got"
else
...
echo "[刷新令牌]"
if [ -n "$RT" ]; then
  rf=$(curl -s -X POST -H 'Content-Type: application/json' -d "{\"refreshToken\":\"$RT\"}" "$GW/api/v1/user/refresh")
  rc=$(printf '%s' "$rf" | jnum code); nt=$(printf '%s' "$rf" | jstr token)
  { [ "$rc" = "0" ] && [ -n "$nt" ]; } && ok "T12 refresh 换新 access(code=0)" || ng "T12 refresh" "0+token" "$rc :: $rf"
else
  ng "T12 refresh(未拿到 refreshToken)" "refreshToken" "empty"
fi
