...
- **🔴/🟡 go-live 最后一段(P10 收)**:① **agent docker 容器仍旧版**→ 生产内助手 SSE 404/F01 格式不符,需当前源**重建 agent 镜像 + `docker compose up -d agent`**(HUB 已重建 agent jar 备用);② **deploy 配置固化**(`D:\InfiniteChatDeploy` 只读,需 sudo/用户:`deploy/.env`+compose+`init-chat-schema.sql`,否则重启丢);③ **S2 浏览器级 F01/§9 E2E**(E2E 栈 Redis `:6399` down P7 起累积 + Win→WSL `:10010` NAT 不可达);④ DLQ lag gauge(L3)。
