...
### 2026-06-30 · P14 v1.0.0 final acceptance:版本化 E2E 57/57 绿 + 生产 Docker 实际映射冲烟 12/12
- 完成:从 `main 9d90ee1` 起独立 worktree `E:\jhw\proj-chatbe-p14` / 分支 `feat/chat-backend-p14`;开工先按要求执行 `git checkout -- 'chat/e2e/*.sh' 'chat/scripts/*'`。已首手通读 `docs/planning/STATUS.md`、`chat/docs/E2E-TESTING.md`、`docs/planning/60-e2e-test-environment.md` 与 `chat/e2e/01/02/03/04/06/07/08/09/10/11/12`、`chat/scripts/runtime-smoke.sh` 等脚本。WSL 一度因 Nacos/进程状态返回 `Wsl/Service/E_UNEXPECTED`,已先从 Windows 侧记录运行态/日志目录后 `wsl --shutdown` 恢复,再重启验收栈。
- 完成(构建/部署):chat `02-build.sh` 从 P14 worktree 源码构建 **BUILD SUCCESS**: `chat-common-1.0.0.jar` + 7 个服务 `*-1.0.0.jar`;agent `09-agent-e2e.sh` 从 P14 agent 源码构建并启动 **`/home/hanwen/projecta-e2e/agent/target/InfiniteChat-Agent-1.0.0.jar`**(`:18080` readiness 200)。E2E 当前 Nacos 取 Docker 发布口 `127.0.0.1:18848`(gRPC `19848`),`01-setup-infra.sh` 已改为尊重 `NACOS_SERVER_ADDR`,命名空间创建复测 `HTTP 200 body=true`。
- 验证(E2E first-hand):全栈 E2E **57/57 全绿**:`04-smoke-test` **PASS=13 FAIL=0**;`06-client-api-smoke` **PASS=10 FAIL=0**;`07-im-smoke` **PASS=14 FAIL=0**;`08-ws-realtime-smoke` **PASS=4 FAIL=0**;`09-agent-e2e` 构建+启动成功且进程命令行确认 `InfiniteChat-Agent-1.0.0.jar`;`10-agent-smoke` **PASS=5 FAIL=0**;`11-assistant-e2e` **PASS=5 FAIL=0**(含 F01 `confirmationRequired + challengeToken` 与持令牌重发一次性放行);`12-envelope-flip-smoke` **PASS=6 FAIL=0**。
- 验证(生产 runtime):当前 WSL Docker 生产栈实际发布为 `infinitechat-gateway 0.0.0.0:11010->10010/tcp`、`infinitechat-agent 0.0.0.0:11011->10011/tcp`、env=`~/p9-deploy/.env`。按实际映射运行 `SMOKE_ENV=~/p9-deploy/.env GATEWAY_PUBLIC_PORT=11010 AGENT_PUBLIC_PORT=11011 bash chat/scripts/runtime-smoke.sh` **PASS=12 FAIL=0**:无 token 401、garbage 401、邮箱注册+登录、IM 发收+真实库落地、历史回显、内助手 SSE、真实 delta、CORS preflight/actual POST、F01 challengeToken + 一次性放行均绿。
- 修复:① `chat/e2e/01-setup-infra.sh` 不再硬编码 `127.0.0.1:8848`,改用 `NACOS_SERVER_ADDR`;② `chat/e2e/09-agent-e2e.sh` 在 Maven build 后重新选择 jar,并优先非 legacy `1.0.0` jar,避免旧 `0.0.1-SNAPSHOT` alias 被先选;③ `runtime-agent-golive.sh`/`runtime-deploy.sh` 同步优先真实版本 jar,移除 `runtime-deploy` 的 legacy 默认路径;④ `runtime-smoke.sh` banner 打印实际 `$GW`,避免传参时仍显示 `:10010`。
- 产出物:`chat/e2e/01-setup-infra.sh`,`chat/e2e/09-agent-e2e.sh`,`chat/scripts/runtime-agent-golive.sh`,`chat/scripts/runtime-deploy.sh`,`chat/scripts/runtime-smoke.sh`,`docs/planning/STATUS.md`。验证:`bash -n` 通过上述 5 个脚本。
- 关键决策:不在本分支修改 Windows host portproxy/外部 Docker 端口绑定;只修 chat-owned scripts 并按当前真实 Docker 映射跑生产冲烟。
- 阻塞:名义 **prod `:10010` 未报绿**。当前 Windows `netsh interface portproxy` 为 `0.0.0.0 10010 -> 192.168.112.154 10010`,WSL/Windows `:10010` 返回 empty reply;该端口由 Windows `svchost` 监听,不是当前 Docker gateway。实际生产 Docker gateway 在 host `:11010` 上已冲烟 12/12 绿。若 release 必须固定公开 `:10010`,需 HUB/用户在宿主网络层重定向该 portproxy 或调整 Docker 发布端口后复跑 `runtime-smoke.sh`。
- 交接:HUB 可采信代码/构建/E2E 57/57 与实际 Docker runtime 12/12 结果;请单独确认 `:10010` 宿主端口归属是否属于 v1.0.0 tag 阻断。
- 待中枢确认:是否把当前生产公开口径从 `:10010` 更新为实际 `:11010`,或由有权限者修 Windows portproxy/compose 映射后再要求 S3 复跑 `runtime-smoke.sh`。
