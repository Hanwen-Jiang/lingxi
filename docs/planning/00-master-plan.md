...
| **D14 登录模型** | **邮箱登录,去手机号/短信**(2026-06-27 拍板) | 身份主体=**邮箱**;支持**邮箱+密码**登录,也支持**邮箱验证码**(注册 + 免密登录)。**移除手机号/短信验证**(`loginCode` 的 SMS 路径删除、注册改邮箱验证码)。`sendMail` 邮箱验证码已有。影响 S3 Auth(register/login/loginCode 改邮箱)+ S2/S4 登录 UI。 | S3,S2,S4 |
...
7. ✅ **登录模型 = 邮箱(去手机号/短信)**(D14):邮箱+密码 与 邮箱验证码(注册/免密登录);移除 SMS。
